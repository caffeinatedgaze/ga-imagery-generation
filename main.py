...
# Image.fromarray(get_best()[0]) # save('tmp/' + 'strokes' + '.png', 'PNG')
...
for i in range(2 ** 64):
    ...
    if i % 250 == 0:
        save('library/_data__' + str(cfg.fitness[get_best()[1]]), get_best()[0])
    if abs(chkpoint_fit - min(cfg.fitness)) >= 100000:
        chkpoint_fit = min(cfg.fitness)
        # Image.fromarray(get_best()).save('tmp/' + 'strokes' + '.png', 'PNG')
        Image.fromarray(get_best()[0]).save('/usr/local/share/jupyter/nbextensions/google.colab/strokes.jpg', 'JPEG')
        save('library/_data__' + str(cfg.fitness[get_best()[1]]), get_best()[0])
